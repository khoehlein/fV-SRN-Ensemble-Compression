...
from data.datasets import OutputMode, DatasetType
from data.datasets.multivariate import (
    MultivariateEnsembleDataStorage,
    MultivariateWorldSpaceDensityData,
    MultivariateWorldSpaceVisualizationData
)
...
from inference.volume import RenderTool, VolumeEvaluator
from training.evaluation import EvaluateWorld, EvaluateMultivariateScreen
...
from training.visualization import MultivariateVisualizer
...
PyrendererOutputParameterization.set_output_mode(OutputMode.MULTIVARIATE)


def build_parser():
    parser = argparse.ArgumentParser()
    StorageManager.init_parser(parser, os.path.split(__file__)[0])
    MultivariateEnsembleDataStorage.init_parser(parser)
    PositionSampler.init_parser(parser)
    MultivariateWorldSpaceDensityData.init_parser(parser)
    MultivariateWorldSpaceVisualizationData.init_parser(parser)
...
def build_dataset(mode: DatasetType, args: Dict[str, Any], volume_data_storage: MultivariateEnsembleDataStorage, volume_evaluator: VolumeEvaluator):
    sampler = PositionSampler.from_dict(args, mode=mode)
    data = MultivariateWorldSpaceDensityData.from_dict(args, volume_data_storage, mode=mode, volume_evaluator=volume_evaluator, position_sampler=sampler)
    return data
...
def build_evaluation_helpers(device, dtype, image_evaluator, loss_screen, loss_world, network, visualization_image_size):
    ...
    evaluator_vis = EvaluateMultivariateScreen(network, image_evaluator, loss_screen, *visualization_image_size, False, False, dtype, device)
...
def main():
    ...
    volume_data_storage = MultivariateEnsembleDataStorage.from_dict(args)
    ...
    visualization_data = MultivariateWorldSpaceVisualizationData.from_dict(args, volume_data_storage, render_tool=render_tool)
    ...
    network = PyrendererSRN.from_dict(
        ...
        output_mode=MultivariateOutputMode(volume_data_storage.num_variables())
    )
    ...
    def run_visualization():
        with torch.no_grad():
            visualizer = MultivariateVisualizer(
                visualization_data.image_size(),
                visualization_data.num_members(),
                volume_data_storage.num_variables(),
