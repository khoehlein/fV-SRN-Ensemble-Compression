...
EXPERIMENT_NAME = 'ensemble/multi_core/retraining/' + folder
...
SCRIPT_PATH = 'experiments/retraining/multi_core/run_training.py'

models_folder = os.path.join(get_output_base_path(), 'ensemble/multi_core/num_channels/', folder, '/results/model')
pth_name = 'model_epoch_50.pth'
run_names = [d for d in sorted(os.listdir(models_folder)) if d.startswith('run')]

PARAMETERS = {
    '--multi-core-loader:pth-path': [os.path.join(models_folder, run_name, pth_name) for run_name in run_names],
