...
from data.datasets.univariate import VolumeDataStorage, WorldSpaceVisualizationData
from data.datasets.univariate.ordered_ensemble_dataset import OrderedWorldSpaceDensityData
from experiments.retraining.multi_core.model_loader import MultiCoreModelLoader
from inference.losses.lossnet import LossFactory
from inference.volume import RenderTool, VolumeEvaluator
from training.evaluation import EvaluateOrderedWorld, EvaluateScreen
...
def build_parser():
    ...
    MultiCoreModelLoader.init_parser(parser)
...
def build_dataset(mode: DatasetType, args: Dict[str, Any], volume_data_storage: VolumeDataStorage, volume_evaluator: VolumeEvaluator):
    ...
    data = OrderedWorldSpaceDensityData.from_dict(args, volume_data_storage, mode=mode, volume_evaluator=volume_evaluator, position_sampler=sampler)
    return data


def build_evaluation_helpers(device, dtype, image_evaluator, loss_screen, loss_world, network, visualization_image_size):
    evaluator_train = EvaluateOrderedWorld(network, image_evaluator, loss_world, dtype, device)
    evaluator_val = EvaluateOrderedWorld(network, image_evaluator, loss_world, dtype, device)
...
def main():
    ...
    model_loader = MultiCoreModelLoader.from_dict(args)
    network, args = model_loader.build_model(
        args,
        member_keys=training_data.ensemble_index,
        dataset_time_keys=training_data.timestep_index,
        output_mode=VolumeEvaluator.output_mode()
    )
    ...
    def run_training():
        ...
        network.train()
        data_loader = training_data.get_dataloader(shuffle=True, drop_last=False)
        ...
        for i, data_tuple in enumerate(data_loader):
            ...
            t_curr = time.time()
            print(f'[INFO] {i + 1} of {num_batches} (Time per batch: {(t_curr - t_start) / (i + 1)})')
